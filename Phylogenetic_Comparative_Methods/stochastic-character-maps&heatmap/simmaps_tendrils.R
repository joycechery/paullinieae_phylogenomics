###############################################################################
# Stochastic Character Mapping for Tendrils
...
state_levels <-c("Absent", "Present")
factor_states <- factor(datas, levels = state_levels)
numeric_states <- as.numeric(factor_states)

# Define color mapping
col_tendrils <- c("grey", "tomato")
names(col_tendrils) <- state_levels 

# Create cor_data using numeric states
cor_data <- data.frame(Taxa = species, State = datas) 

#To get the simmaps, you have two options:
#OPTION 1: if you want to just read in the saved RDS file with the 1000 simmaps for categories, run line 52 THEN SKIP OVER TO 79
simmap.trees_tendrils <- readRDS("simmap.trees_tendrils.RDS")

#OPTION 2: If you want to actually make the simmmaps of tendrils, then run lines  57 ONWARD
...
set.seed(123)
simmap.trees_tendrils <- makeSimmap(tree = pruned_tree, data = cor_data, model = Q, rate.cat = 1, nSim = 1000)
cor_data$State

# Save results
saveRDS(simmap.trees_tendrils, file = "simmap.trees_tendrils.RDS")

# >>> Summarize simmap results <<<
summary_stm_tendrils <- describe.simmap(simmap.trees_tendrils)
print(summary_stm_tendrils)
...
plot_simmap(time_tree = simmap.trees_tendrils[[1]], 
            tree = simmap.trees_tendrils[[1]], 
            simmaps = simmap.trees_tendrils, 
            states = state_levels,
            show.tip.label = T, label.offset = .45,
            lwd = 2.5,
            label.cex = .45,
            colors = col_tendrils, edge.width=0, nt=10001)

nodelabels(pie=pies,cex=0.16,piecol=col_tendrils, lwd=1) 
add.simmap.legend(leg = state_levels, colors = col_tendrils, prompt = TRUE, vertical = TRUE)
title(main = "Tendrils", font.main = 2, line = -1)
